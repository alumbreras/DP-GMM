...
sample_beta_a0 <- function(S, W, init=4){
  ...
  if (is.matrix(W)){
     lb <- TRUE
     xlb <- log(dim(W)[1]-0.9)
     xpoints <- c(xlb, 4, 50, 100, 500) # ok
     xpoints <- c(max(xlb, init-4), init, init+4) # ok
     xpoints <- c(max(xlb, init-10), init, init+10) 
     #xpoints <- c(xlb, 4, 25, 100)
   }
   else{
     xpoints <- c(-4,4,25)
     xpoints <- c(init-10, init, init+10)
     #xpoints <- c(-5,0,15)
     lb <- FALSE
     xlb = 0 # will be ignored since lb is False
  ...
  y <- ars2(1, f_y, f_y_prima, x=xpoints, lb=lb, xlb=xlb, S=S, W=W)
  exp(y)
}
...
# unidimensional?
# S <- rWishart(10, 2, solve(2*W))
# s <- c(0.03728649,  0.03892926,  0.03755899,  0.03945129)
# w <- 26.03153
# sample_beta_a0(s,w)
